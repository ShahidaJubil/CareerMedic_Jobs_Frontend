...
function HospitalProfile() {
  ...
  const handleOpenModal = async (jobData) => {
    ...
    try {
      const jobDataResponse = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/geteach/job/${jobId}`);
              ...
              <Grid item xs={11} sm={6} md={6} lg={5}>
                ...
                <div className="jobList">
                  ...
                  {JobsData.map((item, index) => (
                    ...
                    //   {item.job}
                    // </button>
                    <Button variant="contained" key={index} onClick={() => handleOpenModal(item)}>{item.job}</Button>
                  ))}
                </div>
              </Grid>
            </Grid>
                ...
                <div>
                  ...
                  {selectedJobData.user.map((user, index) => (
                    <Accordion key={index}>
                      ...
                      <AccordionDetails>
                        <Button variant="outlined" color="error" onClick={() => handleRemove(user)}>Remove</Button>
                      </AccordionDetails>
                    </Accordion>
                  ))}
                </div>
